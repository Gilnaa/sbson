...
fn bench_goto_item(c: &mut Criterion) {
    ...
    for i in (0..8000).step_by(71).chain([4095]) {
        ...
        group.bench_function(BenchmarkId::new("goto_borrow_eytzinger", i), |b| {
            b.iter(|| {
        ...
        // group.bench_function(BenchmarkId::new("goto_arc_eytzinger", i), |b| {
        //     b.iter(|| {
        //         let integer = top_arc_eytzinger
        //             .get_value_by_key(&item_name)
        //             .unwrap()
        //             .get_value_by_key("something")
        //             .unwrap()
